...
"""
From https://github.com/yaqwsx/PcbDraw-Lib/blob/master/scripts/create_template.py
Requires PCB_Draw 0.9.0
May be lauched wiht runInDocker.sh the bash will have all the required tools and linked to current dir.

example 
python3 ./create_template.py footprint --front /usr/share/kicad/footprints/TestPoint.pretty/TestPoint_Pad_1.0x1.0mm.kicad_mod TestPoint/TestPoint_Pad_1.0x1.0mm.svg
"""
...
def addOrigin(document):
    ...
    origin.attrib["width"] = str(ki2svg(mm2ki(1))/1000)
    origin.attrib["height"] = str(ki2svg(mm2ki(1))/1000)
    origin.attrib["x"] = "0"
    origin.attrib["y"] = "0"
...
#            viewBox=f"{ki2svg(bb.GetX())/1000} {ki2svg(bb.GetY())/1000} {ki2svg(bb.GetWidth())/1000} {ki2svg(bb.GetHeight())/1000}")
            viewBox=f"{ki2mm(bb.GetX())} {ki2mm(bb.GetY())} {ki2mm(bb.GetWidth())} {ki2mm(bb.GetHeight())}")
    if front:
